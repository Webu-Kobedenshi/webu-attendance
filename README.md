...
木曜 08:43 JST  ─┐
                 ...
                 │   → 選択肢: 遅刻 / 欠席 / 就活（8時間後に Discord 側で自動締切）
                 │
木曜 17:13 JST  ─┤ GitHub Actions (cron) が Poll の投票結果を集計
                 │   → Discord から投票者リストを取得
                 │   → members シートと突合し「遅刻 / 欠席 / 就活 / 未回答」を判定
...
何もしなくてよい。GitHub Actions の cron が毎週木曜に起動し、
`src/activity-schedule.ts` に登録された活動日だけ処理する。
- `Post Attendance Poll`: 木曜 08:43 JST
- `Collect Attendance Poll`: 木曜 17:13 JST（Poll 締切の十数分後）

登録されていない木曜日は投稿・集計ともにスキップする。2026年の確定済み活動日は次の通り。
...
cron は木曜固定のままにしている。登録されていない日や木曜以外で出欠を取りたい場合は、当日に運営が **手動で** Poll 投稿と集計を実行する。手動実行は活動日リストに関係なく動作する。
...
- 行: 部員（チーム → 表示名でソート）
- 列: 活動日（左から古い順）
- セル: `遅`=遅刻 / `×`=欠席 / `就`=就活 / `△`=未回答 / 空=記録なし（過去の `○`=出席も表示）
